...
for k = 1:cellNum
    ...
    for c = 1:size(normR, 1)
        ...
        sortedRU = sort(meanTr(:, 1:20), 'descend');
        [~, idx] = sort(meanTr(:, 1:20), 'descend');
        normRU = dnormR(trNum+1, 1:20);
        normRN = dnormR(trNum+1, 21:end);
        sortedRUi = normRU(:, idx);
        sortedRN = normRN(:, idx);
        strMNa = [strMNa; sortedRN]; strMUa = [strMUa; sortedRUi];
        trNum = trNum+1;
    end
...
xlabel('Stimulus UN rank'), ylabel('Net response(spikes/second)'), title('M2', 'FontSize', 12);
hold off
alpha(.08);
